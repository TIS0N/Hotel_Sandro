...
function invalid_user_mail($user_email) {
  ...
  function create_user($conn, $user_mail, $gender, $first_name, $last_name, $street, $number, $zip_code, $post_town, $password1){
    $sql = "INSERT INTO users(usersEmail, usersAnrede, usersFirstName, usersLastName, usersStreet, usersStreetNumber, usersZipCode, usersPostTown, usersPwd) VALUES (?,?,?,?,?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    ...
    mysqli_stmt_bind_param($stmt, "sssssssss", $user_mail, $gender, $first_name, $last_name, $street, $number, $zip_code, $post_town, $hashed_password);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
