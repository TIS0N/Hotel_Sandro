<?php
//to find out if there is any empty input in form
...
//to find out if there is any empty input when changing password
...
//to find out if the passwords match with one another
...
//to find out if the new passwords are the same so that the correct password will be saved in database
...
//to find out if the old and new passwords are the same 
...
function loginUser($conn, $usersUid, $pwd){
  $usersUidExists = userExists($conn, $usersUid, $usersUid);
    ...
    if ($usersUidExists["active"] === 0) {
        header("location: ../pages/login.php?error=userdeactivated");
        exit();
    }
...
function getUserActiveStatus($conn, $id) {
    ...
    if(!mysqli_stmt_prepare($stmt, $sql)){
        header("location: ../inc/usersList.php?error=statementfailed");
        exit();
    }
    ...
    if($row  = mysqli_fetch_assoc($resData)){
        ...
        mysqli_stmt_close($stmt);
        header("location: ../inc/usersList.php?error=usernotfound");
        exit();
    }
}

//to change user status between active and deactivated 
function toggleActiveUser($conn, $id) {
    ...
    mysqli_stmt_bind_param($stmt, "is", $newActive, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);

    header("location: ../inc/usersList.php");
    exit();
}
